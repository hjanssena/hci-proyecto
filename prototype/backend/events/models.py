...
class EventProfessor(models.Model):
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name='event_professors')
    professor = models.ForeignKey(Professor, on_delete=models.PROTECT, related_name='event_assignments')
    ...
    def __str__(self):
        return f"{self.professor.name} - {self.event.name} ({self.hours}h)"
