...
class _EventFormScreenState extends State<EventFormScreen> {
  ...
  void _validateDates() {
    setState(() {
      _startDateError = _submitted && _startDate == null ? 'Seleccione la fecha de inicio' : null;
  ...
  void _populateFromEvent(Event event) {
    ...
    _professors = event.professors.map((ep) => _ProfessorEntry(
          ...
          Row(
            children: [
              ...
              if (isClone) ...[
                ...
                const InfoTooltip(message: 'Los campos han sido prellenados con los datos del evento original.\nModifique las fechas, profesores y demás datos variables.'),
              ],
            ],
          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(child: _buildDateField('Fecha de inicio', _startDate, (d) { setState(() => _startDate = d); _validateDates(); }, highlight: isClone, errorText: _startDateError)),
                        const SizedBox(width: 16),
                        Expanded(child: _buildDateField('Fecha de fin', _endDate, (d) { setState(() => _endDate = d); _validateDates(); }, highlight: isClone, errorText: _endDateError)),
                        const SizedBox(width: 16),
                        ...
                        const SizedBox(width: 16),
                        SizedBox(width: 120, child: _buildField(
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: DropdownButtonFormField<String>(
                            ...
                            items: Event.modalityLabels.entries.map((e) =>
                              DropdownMenuItem(value: e.key, child: Text(e.value)),
                            ).toList(),
                            onChanged: (v) => setState(() => _modality = v ?? 'PR'),
                          ),
                        ),
                        const SizedBox(width: 16),
                        Expanded(child: _buildField(
                          controller: _locationCtrl,
                          label: _modality == 'VI' ? 'Enlace de acceso' : 'Aula / Ubicación',
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        SizedBox(width: 180, child: _buildField(
                        ...
                        SizedBox(width: 180, child: _buildField(
                          ...
                          validator: (v) {
                            ...
                            final n = int.tryParse(v.trim());
                            if (n == null || n <= 0) return 'Debe ser mayor a 0';
                            final maxCap = int.tryParse(_maxCapCtrl.text.trim()) ?? 0;
                            if (maxCap > 0 && n != null && n > maxCap) return 'No puede exceder la capacidad';
                            return null;
                          },
                        )),
                        const SizedBox(width: 16),
                    ...
                    Row(
                      children: [
                        SizedBox(width: 180, child: CheckboxListTile(title: const Text('Puntos EPC'), value: _epcPoints, onChanged: (v) => setState(() => _epcPoints = v ?? false), controlAffinity: ListTileControlAffinity.leading, contentPadding: EdgeInsets.zero)),
                        SizedBox(width: 180, child: CheckboxListTile(title: const Text('Por contrato'), value: _byContract, onChanged: (v) => setState(() => _byContract = v ?? false), controlAffinity: ListTileControlAffinity.leading, contentPadding: EdgeInsets.zero)),
                        if (_byContract) ...[
                          const SizedBox(width: 16),
                          Expanded(child: _buildField(
                            controller: _orgCtrl,
                            label: 'Organización',
                            validator: (v) => _byContract && (v == null || v.trim().isEmpty) ? 'La organización es obligatoria' : null,
                          )),
                        ],
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        const InfoTooltip(message: 'Asigne uno o más profesores al evento con las horas correspondientes.'),
                        const Spacer(),
                        TextButton.icon(
                          onPressed: () {
                          ...
                          children: [
                            Expanded(
                              ...
                              child: DropdownButtonFormField<int>(
                                ...
                                items: vm.professors.map((p) =>
                                  DropdownMenuItem(value: p.id, child: Text(p.name)),
                                ).toList(),
                                onChanged: (v) {
                                  setState(() {
                                    prof.professor = vm.professors.firstWhere((p) => p.id == v);
                                  });
                                },
                            ...
                            SizedBox(
                              width: 120,
                              child: TextFormField(
                                initialValue: prof.hours > 0 ? prof.hours.toString() : '',
                                decoration: InputDecoration(
                                  labelText: 'Horas',
                                  border: const OutlineInputBorder(),
                                  filled: isClone,
                                ...
                                keyboardType: TextInputType.number,
                                onChanged: (v) => prof.hours = int.tryParse(v) ?? 0,
                                validator: (v) {
                                  if (v == null || v.trim().isEmpty) return 'Obligatorio';
                                  final n = int.tryParse(v.trim());
                            ...
                            IconButton(
                              ...
                              onPressed: () => setState(() => _professors.removeAt(idx)),
                            ),
                          ],
                        ),
                      );
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 16),
                        ElevatedButton(
                          onPressed: vm.state == EventFormState.saving ? null : _saveEvent,
  ...
  Widget _buildDateField(String label, DateTime? value, ValueChanged<DateTime> onChanged, {bool highlight = false, String? errorText}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      ...
      children: [
        InkWell(
          ...
          child: InputDecorator(
            ...
            child: Text(
              value != null ? '${value.year}-${value.month.toString().padLeft(2, '0')}-${value.day.toString().padLeft(2, '0')}' : 'Seleccionar fecha',
  ...
  void _saveEvent() {
    ...
    final datesValid = _startDateError == null && _endDateError == null && _startDate != null && _endDate != null;
    ...
    vm.saveEvent(data, eventId: isEdit ? widget.eventId : null).then((error) {
      if (error == null && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(isEdit ? 'Evento actualizado exitosamente' : 'Evento creado exitosamente'), backgroundColor: Colors.green),
        );
        widget.onBack();
      }
    });
